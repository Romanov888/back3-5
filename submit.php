...
$_SESSION['data'] = [
    ...
    if (empty($errors)) {
        ...
        try {
            $stmt = $db->prepare("INSERT INTO users (name, email, birth_year, gender, bio, contract) VALUES (?, ?, ?, ?, ?, ?)");
            $stmt->execute([$name, $email, $birth_year, $gender, $bio, $contract]);
            ...
            $stmt = $db->prepare("SELECT id FROM languages WHERE language_name = ?");
            foreach ($languages as $language) {
            $stmt->execute([$language]);
            $language_id = $stmt->fetchColumn();
 
            $stmt2 = $db->prepare("INSERT INTO user_languages (user_id, language_id) VALUES (?, ?)");
            ...
            setcookie('languages', implode(',', $languages), $cookie_expires);
